...
class ThemeSuggestion(BaseModel):
    ...
    name: str = Field(..., description="Theme name (e.g., 'Water-Liquid', 'Childhood')")
    ...
    evidence: str = Field(..., description="Brief textual evidence for this theme")
...
class ThemeDetectionResult(BaseModel):
    ...
    themes: List[ThemeSuggestion] = Field(
        default_factory=list, description="List of detected themes with confidence scores"
    )
