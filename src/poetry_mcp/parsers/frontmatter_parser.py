...
def parse_poem_file(file_path: Path, vault_root: Path) -> Poem:
    ...
    try:
        ...
        raise FrontmatterParseError(
            f"Failed to create Poem object for {file_path}: {e}"
        )
...
def extract_frontmatter(content: str, file_path: Path) -> tuple[dict, str]:
    ...
    try:
        ...
        if not isinstance(frontmatter, dict):
            raise FrontmatterParseError(
                f"Frontmatter in {file_path} is not a valid YAML object"
            )
    except yaml.YAMLError as e:
        raise FrontmatterParseError(
            f"Invalid YAML in {file_path} frontmatter: {e}"
        )
...
def detect_form(content: str) -> str:
    ...
    if len(lines) >= 3:
        starting_words = [line.strip().split()[0].lower() if line.strip().split() else ''
                         for line in lines]
